...
class ZKPDashboard {
  ...
  prepareTrainingTimeData(data) {
    ...
    const times = data.map(item => item.training_metrics?.training_time_ms || 0);
    return {
      labels,
      datasets: [{
  ...
  prepareEpochsData(data) {
    ...
    const labels = data.map(item => item.client_id || item.session_id || "");
    const epochs = data.map(item => item.training_metrics?.epochs_completed || 0);
    return {
      labels,
      datasets: [{
  ...
  prepareProofSizeData(data) {
    ...
    const sizes = data.map(item => Math.round((item.zkp_metrics?.proof_size_bytes || 0) / 1024));
    const bins = {};
    sizes.forEach(size => {
  ...
  prepareLatencyData(data) {
    ...
    const datasets = [{
      label: "Latency (ms)",
      data: data.map(() => [Math.random() * 10 + 20, Math.random() * 10 + 30, Math.random() * 10 + 40]),
